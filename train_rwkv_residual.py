...
while(1):
    ...
    if curr_batch % grad_accum_epochs == 0:
        ...
        if opt_steps % steps_per_printout == 0:
            # print training info
            print(f'tokens: {opt_steps * batch_size * grad_accum_epochs}, mse loss: {loss.cpu() * grad_accum_epochs}, avg step time: {(time.time() - start_time) / steps_per_printout}, tps: {(batch_size * grad_accum_epochs * steps_per_printout)/(time.time() - start_time)}')
